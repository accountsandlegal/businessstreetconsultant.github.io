 ...
 * Set dependent style
 */
$sc_dir = FAMULUS_T_URI . '/aheto/portfolio-nav/';
wp_enqueue_style('famulus-portfolio-nav-modern', $sc_dir . 'assets/css/cs_layout1.css', null, null);
			...
			$prev = get_previous_post_link(
				...
				'<div class="portfolio-nav__dir-image"><img src="' . $prev_post_image . '" alt="' . $prev_post_alt . '" class="js-bg"></div>                 
					  <h5 class="portfolio-nav__dir-title">' . $prev_post->post_title .
							'<p>' . __( 'Prev ', 'famulus' ) . $cs_prev_next . '</p>
					  </h5>',
				false,
				'',
		...
		if ( !empty($next_post) ) {
			...
			$prev = get_next_post_link(
				...
				'<h5 class="portfolio-nav__dir-title">' . $next_post->post_title .
						'<p>' . __( 'Next ', 'famulus' ). $cs_prev_next . '</p>
					  </h5>
